...
function cEmail($email, $campo, $errores){
...
function cFecha(string $fechaX,string $campo,array $errores){
    ...
    if(checkdate($mes,$dia,$anyo)){
        $fechaUnix=mktime(0,0,0,$mes,$dia,$anyo);
        return $fechaUnix;
    }else{
        $errores[$campo]= "Hay un error en el campo $campo";
        return false;
    }
}
function cContrasenya(string $text, string $campo, array &$errores){
    if ((preg_match("/^[a-z_][0-9_a-z\*_\$\-&\/\\\+]{1,14}$/ui", sinTildes($text)))) {
        return true;
    }
    $errores[$campo] = "Error en el campo $campo";
    return false;
}
function cUsuarios(string $text, string $campo, array &$errores){
    if ((preg_match("/^[a-z_][0-9_a-z]{1,11}$/ui", sinTildes($text)))) {
        return true;
    }
    $errores[$campo] = "Error en el campo $campo";
    return false;
...
function generaToken($mail){
    $parteUsuario=explode("@",$mail);
    $usuario=$parteUsuario[0];
    $token=$usuario.str_shuffle($usuario);
    return $token;
}
