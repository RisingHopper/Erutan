...
class Erutan extends Modelo {
    ...
    public function verRanking() {
        $consulta = "SELECT idUsuario, SUM(respuestasAcertadas) FROM ranking_usuarios GROUP by idUsuario LIMIT 3";
        $result = $this->conexion->prepare($consulta);
        $result->bindParam(':titulo', $idPublicacion);
        $result->execute();
        return $result->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function insertarPublicacion($titulo, $tituloPost, $textoCard, $contenido,$categoria,$imagen,$contenidoCarrusel){
        $consulta="INSERT INTO publicaciones_contenido_web (titulo, tituloPost, textoCard,contenido,categoria,imagen,contenidoCarrusel) VALUES (:titulo, :tituloPost, :textoCard,:contenido, :categoria, :imagen, :contenidoCarrusel)";
        $result = $this->conexion->prepare($consulta);
        $result->bindParam(':titulo', $titulo);
        $result->bindParam(':tituloPost', $tituloPost);
        $result->bindParam(':textoCard', $textoCard);
